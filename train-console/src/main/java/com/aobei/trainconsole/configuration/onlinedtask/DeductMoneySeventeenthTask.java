...
@Configuration
public class DeductMoneySeventeenthTask {
	...
	private void extractData() {
        //先将待结算变成已结算
        FallintoDeductMoneyExample fallintoDeductMoneyExamples = new FallintoDeductMoneyExample();
        fallintoDeductMoneyExamples.or().andStatusEqualTo(1);
        List<FallintoDeductMoney> fallintoDeductMonies = fallintoDeductMoneyService.selectByExample(fallintoDeductMoneyExamples);
        ...
        long autoIncrId = idGenerator.getAutoIncrNum("DSVT"+date);
        if (autoIncrId != 1){
            return;
        }
