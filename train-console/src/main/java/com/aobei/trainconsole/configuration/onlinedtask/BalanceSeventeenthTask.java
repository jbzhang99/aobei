...
@Configuration
public class BalanceSeventeenthTask {
	...
	private void extractData(){
        // 没有挂起的数据自动处理为“已结算”
        BalanceOrderExample balanceOrderExamples = new BalanceOrderExample();
        balanceOrderExamples.or().andStatusEqualTo(1);
        List<BalanceOrder> balanceOrderList = this.balanceOrderService.selectByExample(balanceOrderExamples);
        if(!balanceOrderList.isEmpty()){
            balanceOrderList.stream().forEach(balanceOrder -> {
        ...
        long autoIncrId = idGenerator.getAutoIncrNum("BSVT"+date);
        if (autoIncrId != 1){
            return;
        }
        ...
        if (!moneyList.isEmpty()) {
            ...
            List<StepData> newStepDataList=new ArrayList<>();
            mapList.forEach((String k, Map<String, List<ServiceUnit>> v) ->{
                Map<String, List<ServiceUnit>> stringListMap = mapList.get(v);
