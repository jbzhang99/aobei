...
@Configuration
public class FineMoneySeventeenthTask {
	...
	private void extractData() {
        //先将待结算变成已结算
        FallintoFineMoneyExample fallintoFineMoneyExamples = new FallintoFineMoneyExample();
        fallintoFineMoneyExamples.or().andStatusEqualTo(1);
        List<FallintoFineMoney> fallintoFineMonies = fallintoFineMoneyService.selectByExample(fallintoFineMoneyExamples);
        ...
        long autoIncrId = idGenerator.getAutoIncrNum("FSVT"+date);
        if (autoIncrId != 1){
            return;
        }
    ...
    public FallintoFineMoney combine(LocalDate localDate,FineMoney fineMoney) {
        ...
        fallintoFineMoney.setBalance_cycle(localDate.getYear() + month + "17");//结算期
