...
@Component
public class ExportDeductMoneyBalanceAndToOss implements Callable<Integer>{
	...
	private int type;
	private FallintoDeductMoneyExample fallintoDeductMoneyExample;
	...
	public FallintoDeductMoneyExample getFallintoDeductMoneyExample() {
		return fallintoDeductMoneyExample;
	}

	public void setFallintoDeductMoneyExample(FallintoDeductMoneyExample fallintoDeductMoneyExample) {
		this.fallintoDeductMoneyExample = fallintoDeductMoneyExample;
	}
	...
	public Integer call() throws Exception {
		...
		String titleName[] ={type==1?"结算期":"","订单号","服务单号","扣款单号","类型",
				"订单生成时间","扣款发起时间","扣款完成时间","订单金额","扣款金额",
				"合伙人","合伙人ID","合伙人级别","合作起日期","合作止日期","状态"};


		List<FallintoDeductMoney> list = fallintoDeductMoneyService.selectByExample(fallintoDeductMoneyExample);
		...
		List<Object[]> dataList = list.stream().map( n->{
			...
			Object[] obj = {n.getStatus()==1?n.getBalance_cycle():"",
					...
					n.getDeduct_money_id()==null?"":n.getDeduct_money_id().toString(),
					"扣款",
					n.getOrder_create_datetime()==null?"":sd.format(n.getOrder_create_datetime()),
					n.getDeduct_money_create_datetime()==null?"":sd.format(n.getDeduct_money_create_datetime()),
					n.getDeduct_confirm_datetime()==null?"":sd.format(n.getDeduct_confirm_datetime()),
					n.getPrice_pay()==null?"":(n.getPrice_pay()*0.01),
					n.getDeduct_money()==null?"":(n.getDeduct_money()*0.01),
